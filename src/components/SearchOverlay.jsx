...
export default function SearchOverlay({ isOpen, onClose }) {
  ...
  const handleSuggestionClick = (item) => {
    if (item.type === 'category') {
      ...
      if (item.soldOut) {
        navigate(`/category/archives?product=${encodeURIComponent(item.title)}`);
      } else {
        // Navigate to the specific category or product detail page
        navigate(`/category/${item.categorySlug || item.category}?product=${encodeURIComponent(item.title)}`);
      }
    }
    onClose()
  }
